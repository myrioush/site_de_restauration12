  ...
  if (isset($_GET['id']) && !empty($_GET['id'])) 
    {
    	$db = Database::connect();
    	$id=(int) $_GET['id'];
    	$statement = $db->prepare("SELECT items.id, items.name, items.description, items.price, items.image, categories.name AS category FROM items LEFT JOIN categories ON items.category = categories.id WHERE items.id =2");
